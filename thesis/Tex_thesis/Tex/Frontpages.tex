...
期权定价是金融工程的核心问题之一。Black-Scholes-Merton（BSM）模型、常弹性方差（Constant Elasticity of Variance，CEV）模型和Heston随机波动率模型是三大经典期权定价框架，分别适用于不同的市场假设。传统数值方法（有限差分、有限元）在求解高维偏微分方程（Partial Differential Equation，PDE）时面临维数灾难，而现有物理信息神经网络（Physics-Informed Neural Network，PINN）研究大多针对单一模型，缺乏多模型统一求解的方法。
...
实验结果表明，统一PINN在BSM模型上达到MAE=0.0005（ATM区域相对误差0.0009\%），在CEV模型上达到MAE=0.032（ATM相对误差0.79\%），在Heston模型上达到MAE=0.102（ATM相对误差0.599\%）；三种模型的ATM相对误差均低于1\%，统一框架在BSM和CEV上的精度优于独立训练的PINN基线。LLM路由层在20个测试用例上实现了100\%的模型选择准确率。单次推断耗时约2ms，远低于传统有限差分法的约500ms。
...
Experimental results show that the unified PINN achieves MAE = 0.0005 on BSM (ATM relative error 0.0009\%), MAE = 0.032 on CEV (ATM relative error 0.79\%), and MAE = 0.102 on Heston (ATM relative error 0.599\%); all three models achieve ATM relative errors below 1\%, and the unified framework outperforms independently trained PINN baselines on BSM and CEV. The LLM routing layer achieves 100\% model selection accuracy on 20 test cases. Single inference takes approximately 2ms, far below the approximately 500ms of traditional finite difference methods.
