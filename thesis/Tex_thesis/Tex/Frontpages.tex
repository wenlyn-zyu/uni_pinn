...
实验结果表明，统一PINN在三种模型的平值附近区域相对误差均低于1\%，统一框架在BSM和CEV上的精度优于独立训练的PINN基线。LLM路由层在全部测试用例上实现了100\%的模型选择准确率。单次推断耗时约2ms，具备实时定价能力。
...
Experimental results show that the unified PINN achieves ATM relative errors below 1\% across all three models, and outperforms independently trained PINN baselines on BSM and CEV. The LLM routing layer achieves 100\% model selection accuracy on all test cases. Single inference takes approximately 2ms, enabling real-time pricing.
