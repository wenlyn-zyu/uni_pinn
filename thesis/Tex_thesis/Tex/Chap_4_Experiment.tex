...
\begin{figure}[htbp]
  ...
  \caption{三大模型期权价格曲线（$t=0$，$S\in[50,250]$，$K=100$）。实线为参考解，虚线为统一PINN预测；竖虚线标记平值点（$S=K$）。三种模型的预测曲线与参考解高度吻合。}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrrrrr}
    \hline
    到期日 & $T$(年) & $n$ & 校准BSM & 校准Heston & PINN-BSM & PINN-Heston & 参数化PINN \\
    \hline
...
\begin{figure}[htbp]
  ...
  \caption{独立PINN与统一PINN价格曲线对比（$t=0$，$S\in[50,250]$）。实线为参考解，虚线为统一PINN，点线为独立PINN；竖虚线标记平值点。}
  \label{fig:compare}
\end{figure}
