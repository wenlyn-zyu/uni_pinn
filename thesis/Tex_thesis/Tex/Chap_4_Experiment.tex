...
\begin{figure}[htbp]
  ...
  \caption{统一PINN（v16）训练损失曲线（30000步，54个参数变体）。总损失与PDE残差损失（$\mathcal{L}_\mathrm{pde}$）同步下降；边界条件损失（$\mathcal{L}_\mathrm{bc}$）和数据锚点损失（$\mathcal{L}_\mathrm{data}$）在训练早期快速收敛至$10^{-5}$量级。余弦退火调度使损失在后期持续缓慢下降。}
