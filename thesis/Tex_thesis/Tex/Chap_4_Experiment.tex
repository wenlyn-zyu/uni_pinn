...
图~\ref{fig:training_loss}展示了统一PINN（v15版本，与v16训练过程相似）的训练损失曲线。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    \hline
    BSM & 0.0003 & 0.0004 & 0.0010\% \\
    CEV ($\beta=0.5$) & 0.032 & 0.045 & 0.79\% \\
    Heston & 0.053 & 0.063 & 0.74\% \\
    \hline
  \end{tabular}
\end{table}

三大模型的精度均达到实用水平。v16版本（unified\_v16\_gl）在BSM上达到近乎完美的精度：BSM的MAE仅为0.0003，RelMAE$_\text{ATM}$=0.0010\%，ATM RelMSE$=1.37\times10^{-10}$，得益于软掩码+BS基线在$\xi=0$时的完美退化；CEV（$\sigma=0.20$，$\beta=0.5$）的MAE=0.032，RelMAE$_\text{ATM}$=0.79\%，ATM RelMSE$=1.06\times10^{-4}$，精度良好；Heston的RelMAE$_\text{ATM}$为0.74\%，ATM RelMSE$=5.93\times10^{-5}$，精度良好。三种模型的ATM相对误差均低于1\%，统一框架在标准参数集下表现均衡。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrrr}
    \hline
    到期日 & $T$(年) & $n$ & BSM & Heston & PINN-BSM & PINN-Heston \\
    \hline
    Jun 05 2026 & 0.060 & 197 & 1.78 & \textbf{0.34} & 1.78 & 15.64 \\
    Jun 30 2026 & 0.129 & 211 & 1.46 & \textbf{0.14} & 1.46 & 34.30 \\
    Aug 21 2026 & 0.271 &  81 & 1.64 & \textbf{0.13} & 1.64 & 73.59 \\
    Sep 30 2026 & 0.381 & 145 & 2.88 & \textbf{0.18} & 2.88 & 10.66 \\
    Dec 31 2026 & 0.633 & 151 & 3.30 & \textbf{0.30} & 3.30 &  4.98 \\
    Mar 19 2027 & 0.847 &  92 & 3.77 & \textbf{0.19} & 3.77 &  6.89 \\
    Dec 17 2027 & 1.595 &  98 & 5.27 & \textbf{0.29} & 5.27 &  7.47 \\
    Dec 15 2028 & 2.592 &  98 & 6.50 & \textbf{0.56} & 6.50 &  8.93 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llccc}
    ...
    \hline
    独立BSM-PINN   & BS解析解       & 0.087 & 0.114 & 1.590\% \\
    独立CEV-PINN   & Schroder解析解 & 0.082 & 0.107 & 6.053\% \\
    Hainaut \& Casas (2024) & GL半解析解 & --- & --- & 9.169\%* \\
    \hline
    统一PINN（BSM）   & BS解析解       & 0.0003 & 0.0004 & 0.0010\% \\
    统一PINN（CEV）   & Schroder解析解 & 0.032 & 0.045 & 0.790\% \\
    统一PINN（Heston）& GL半解析解     & 0.053 & 0.063 & 0.737\% \\
    \hline
  \end{tabular}

  {\footnotesize *Hainaut模型经put-call parity转换为CALL价格；仅在$S/K\geq 0.85$区域计算（RelMAE=9.169\%）。$\theta=0.04$略低于训练下限$0.062$，属轻微OOD。独立CEV-PINN的RelMAE$_\text{ATM}$在全区间$S\in[60,160]$计算（含深度虚值），与第~\ref{chap:related}章复现结果（仅统计$V^*>0.5$区域，RelMAE=1.69\%）评估范围不同。}
\end{table}

从表~\ref{tab:compare}可以看出，统一PINN在三种模型上的精度均显著优于对应的独立PINN：
BSM的RelMAE$_\text{ATM}$从1.590\%降至0.0010\%（提升约1590倍），
CEV的RelMAE$_\text{ATM}$从6.053\%降至0.790\%（提升约8倍），
Heston的RelMAE$_\text{ATM}$从9.169\%降至0.737\%（提升约12倍）。
统一框架通过软掩码机制和数据锚点的协同作用，在三种模型上均实现了精度提升，三种模型的ATM相对误差均低于1\%。
...
统一PINN（v16）在三大模型上均达到实用精度：BSM（ATM RelMSE$\approx 0$）、CEV（ATM RelMSE$=1.06\times10^{-4}$）、Heston（ATM RelMSE$=5.93\times10^{-5}$），三种模型的ATM相对误差均低于1\%。消融实验验证了软掩码、加法参数化和数据锚点三个设计组件的必要性。LLM路由层在20个测试用例上实现了100\%的模型选择准确率。真实市场回测（SPY期权链，2824个合约，23个到期日，详细分析8个代表性到期日）表明：校准Heston在中长期期权上显著优于BSM（以Aug 21 2026为例，Heston MAE=\$0.13，BSM MAE=\$1.64）；PINN-BSM在训练参数范围内具有优异的泛化能力；PINN-Heston因真实市场参数超出训练范围且受制于网络容量瓶颈，定价误差较大，进一步的架构改进是重要的未来工作方向。
