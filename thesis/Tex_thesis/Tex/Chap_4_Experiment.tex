...
PINN-BSM的MAE与校准BSM几乎相同（差距$<0.1$美元，$T<0.4$年），表现出优异的泛化能力。v15版本的BSM训练参数覆盖$\sigma\in[0.13,0.30]$，而真实市场的$\sigma^*$在0.13至0.17之间，恰好落在训练范围内，因此PINN-BSM能够精确复现校准BSM的定价结果，验证了统一框架的参数泛化性。
...
独立Heston-PINN在此参数集下重新训练，引入GL数据锚点机制以确保收敛（见第~\ref{subsec:reproduction}节）。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llccc}
    ...
    独立Heston-PINN & GL半解析解    & 0.383 & 0.491 & 9.91\% \\
    \hline
...
从表~\ref{tab:compare}可以看出，统一PINN在三种模型上的精度均显著优于对应的独立PINN：
BSM的RelMAE从2.98\%降至0.065\%（提升约46倍），CEV从1.88\%降至0.887\%（提升约2.1倍），
Heston的MAE从0.383降至0.036（降幅91\%）。
独立Heston-PINN的高误差（RelMAE=9.91\%）反映了纯PDE约束在二维退化方程上的收敛困难；
...
统一PINN（v15）在三大模型上均达到实用精度：BSM（MAE=0.021，ATM相对误差0.21\%）、CEV（MAE=0.065，ATM相对误差0.81\%）、Heston（MAE=0.034，ATM相对误差0.38\%）。消融实验验证了软掩码、加法参数化和数据锚点三个设计组件的必要性。LLM路由层在20个测试用例上实现了100\%的模型选择准确率。真实市场回测（SPY期权链，2394个合约，34个到期日）表明：校准Heston和CEV在中长期期权（$T>0.25$年）上显著优于BSM；PINN-BSM在训练参数范围内具有优异的泛化能力；PINN-Heston因真实市场参数超出训练范围且受制于网络容量瓶颈，定价误差较大，进一步的架构改进是重要的未来工作方向。
