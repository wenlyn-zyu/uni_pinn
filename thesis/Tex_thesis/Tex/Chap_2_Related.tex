...
\textbf{Hainaut \& Casas参数化PINN。}严格复现Hainaut和Casas\cite{hainaut2024heston}的参数化PINN方法，采用跳跃连接网络（SkipNet）结构：4层$\times$256神经元，每层接收前一层输出与原始输入的拼接（skip connection），输入为9维向量$(t, S, v, r, \kappa, \theta, \xi, \rho, T)$，经z-score归一化后输入网络，输出为PUT期权价格（以$K$为单位）。训练参数范围：$S\in[20,180]$，$v\in[0.032,0.52]$，$r\in[0.01,0.07]$，$\kappa\in[0.5,2.0]$，$\theta\in[0.062,0.42]$，$\xi\in[0.1,0.9]$，$\rho\in[-0.8,0.8]$，$T\in[0.1,5]$。采用4阶段Adam训练：学习率依次为$0.005/0.002/0.001/0.0001$，各阶段训练$500/1000/1000/1000$轮，内部配点$N_D=20000$，终值配点$N_T=5000$，下边界配点$N_\text{low}=5000$。该模型以PUT期权为输出，通过put-call parity转换为CALL价格。评估参数集中$\theta=0.04$略低于训练下限$0.062$，属轻微域外推断（OOD）；原论文Table~7在训练范围内评估实值PUT，报告相对误差约$4.4\%$，本文结果与之一致。

然而，Hainaut \& Casas方法的参数化范围对统一框架所需的参数集（$r=0.05$，$\kappa=2.0$，$\theta=0.04$，$\xi=0.3$，$\rho=-0.7$）存在轻微OOD问题（$\theta=0.04$低于训练下限$0.062$），且该方法仅输出PUT价格，需经put-call parity转换，在深度虚值区域引入额外误差。为解决这一问题，本文在独立Heston-PINN中引入\textbf{数据锚点机制}：在$t=0$处预计算GL特征函数半解析价格作为监督信号，以权重$w_\text{data}=100$加入损失函数，将解锁定在正确分支上，从而在任意参数集下均能稳定收敛。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccc}
    ...
    Hainaut \& Casas (2024) & GL半解析解 & --- & 9.169\%* & 参数化PINN，4层$\times$256，Tanh，skip connection \\
    \hline
  \end{tabular}

  {\footnotesize *Hainaut模型以PUT为输出，经put-call parity转换为CALL价格，仅统计$S/K\geq 0.85$区域（RelMAE=9.169\%）。}
\end{table}
