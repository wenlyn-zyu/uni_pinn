...
\end{equation}
当$\xi=0$时，$a=\sigma^2 S^{2\beta}$，精确退化为BSM/CEV扩散项；当$\xi=0.3$时，$a\approx vS^2$，精确对应Heston扩散项。软掩码的连续插值特性使网络可以学习到平滑的参数依赖关系，避免了硬切换在$\xi$附近产生的梯度不连续问题（消融实验见第~\ref{sec:ablation}节）。
