...
class UnifiedNet(nn.Module):
    ...
    def forward(self, S_n, v_n, t_n, lam, S_raw, t_raw, K, T, r):
        ...
        net 学习相对于 BS 基线的绝对修正量（以 K 为单位）。
        当 V_bs→0 时（深度 OTM），net 仍可输出非零修正，解决乘法参数化的退化问题。
        """
        sigma = lam[:, 0:1]
        xi    = lam[:, 4:5]
        ...
        raw = self.net(x)
        # 加法修正：K 作为缩放因子使 net 输出量级约为 0~0.1
        return V_bs + K * raw
...
class UnifiedPINN:
    ...
    def _sample_batch(self, n_per_model: int = 5000):
        """从每个模型均匀采样配点，拼接成混合批次。
        第一个模型（BSM）用 n_per_model，后续模型用 2x 以平衡梯度竞争。
        """
        S_list, v_list, t_list, lam_list = [], [], [], []
        for i, p in enumerate(self.param_list):
            n = n_per_model if i == 0 else n_per_model * 2
