...
class UnifiedNet(nn.Module):
    ...
    def forward(self, S_n, v_n, t_n, lam, S_raw, t_raw, K, T, r):
        ...
        # 等效波动率：
        #   BSM (beta=1): sigma_eff = sigma
        #   CEV (beta<1): sigma_eff = sigma * (S/K)^(beta-1)，ATM 局部等效波动率
        #   Heston (xi>0): sigma_eff = sqrt(v)
        mask      = torch.tanh(xi / 0.05) ** 2
        v_approx  = torch.clamp(v_n, min=1e-6)   # v_n ≈ v（v_max=1 时精确）
        # CEV 局部等效波动率：在 S=K 附近 sigma_cev ≈ sigma，远离时有弹性修正
        S_over_K  = torch.clamp(S_raw / K, min=1e-4)
        sigma_cev = sigma * S_over_K ** (beta - 1.0)
        sigma_cev = torch.clamp(sigma_cev, min=1e-4, max=2.0)
        sigma_eff = (1 - mask) * sigma_cev + mask * torch.sqrt(v_approx)
