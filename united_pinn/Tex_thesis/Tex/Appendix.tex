...
\begin{verbatim}
united_pinn/
├── unified_pinn.py          # 统一PINN核心模型
├── llm_router.py            # LLM路由层
├── reference_solvers.py     # 参考解（BSM/CEV/Heston）
├── train.py                 # 训练脚本
├── evaluate.py              # 评估脚本
├── utils.py                 # 工具函数（归一化、采样等）
└── configs/
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ll}
    ...
    批大小（配点数$N_c$） & 3000（每模型1000）\\
    边界点数$N_b$ & 300 \\
    数据锚点数$N_d$ & 60（每模型20）\\
    优化器 & Adam（$\beta_1=0.9$，$\beta_2=0.999$）\\
    初始学习率 & $1\times 10^{-3}$ \\
    学习率调度 & StepLR，每5000步$\times 0.5$ \\
    总训练步数 & 20000 \\
    损失权重$(w_\text{pde},w_\text{bc},w_\text{data})$ & $(1, 10, 100)$ \\
    软掩码阈值 & 0.05 \\
    最后一层初始化 & $\mathcal{N}(0, 0.001^2)$ \\
    BSM参数范围（训练） & $\sigma\in[0.1,0.5]$ \\
    CEV参数范围（训练） & $\sigma\in[0.1,0.5]$，$\beta\in[0.3,0.9]$ \\
    Heston参数范围（训练） & $\kappa\in[1,5]$，$\theta\in[0.01,0.09]$，\\
                           & $\xi\in[0.1,0.5]$，$\rho\in[-0.9,-0.3]$ \\
    \hline
  \end{tabular}
\end{table}
