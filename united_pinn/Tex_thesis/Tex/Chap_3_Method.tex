...
本文引入DeepSeek LLM作为智能路由层，使用户可以通过自然语言描述期权需求，由LLM自动完成模型选择和参数提取，无需手动指定模型类型和数值参数。系统提示词将LLM角色定义为"期权定价参数提取器"，规定模型选择规则（仅提供$\sigma$则选BSM，提及$\beta$或"CEV"则选CEV，提供$\kappa,\theta,\xi,\rho,v_0$或提及"Heston"则选Heston，默认BSM），并通过\texttt{response\_format: json\_object}强制输出合法JSON。缺失参数使用美股市场典型默认值（$\sigma=0.2$，$\kappa=2.0$，$\theta=0.04$，$\xi=0.3$，$\rho=-0.7$），系统对越界参数截断至合法范围后传入统一PINN求解器。完整流程为：用户输入自然语言描述，LLM提取参数并选择模型，统一PINN前向传播返回期权价格（约2ms）。
